...
bool om_decoder_next_data_read(const OmDecoder_t *decoder, OmDecoder_dataRead_t* data_read, const void* index_data, uint64_t index_data_size, OmError_t* error) {
    ...
    {
        const size_t thisLutChunkElementCount = OM_MIN((lutChunk + 1) * LUT_CHUNK_COUNT, number_of_chunks+1) - lutChunk * LUT_CHUNK_COUNT;
    ...
    while (true) {
        ...
        if (nextLutChunk != lutChunk) {
            const size_t nextLutChunkElementCount = OM_MIN((nextLutChunk + 1) * LUT_CHUNK_COUNT, number_of_chunks+1) - nextLutChunk * LUT_CHUNK_COUNT;
